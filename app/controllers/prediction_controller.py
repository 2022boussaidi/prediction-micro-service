...
from app.services.prediction_service import predict_count_service, predict_service
...
@prediction_blueprint.route('/predict/<model>', methods=['POST'])
def predict(model):
    zipkin_attrs = ZipkinAttrs(
        trace_id=request.headers.get('X-B3-TraceID'),
        span_id=request.headers.get('X-B3-SpanID'),
    ...
    with zipkin_span(
        ...
        return predict_service(model, request.get_json())
